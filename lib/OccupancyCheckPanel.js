...
$pyjs['loaded_modules']['OccupancyCheckPanel'] = function (__mod_name__) {
	...
	, 1, [null,null,['self']]);
		...
		$method = $pyjs__bind_method2('onClick', function(sender) {
			...
			var $add21,$add22,$pyjs_try_err,error;
			try {
				...
				if (($pyjs_try_err_name == $p['Exception']['__name__'])||$p['_isinstance']($pyjs_try_err,$p['Exception'])) {
					error = $pyjs_try_err;
					$m['ErrorMessage']($p['__op_add']($add21='Exception thrown: ',$add22=$p['str'](error)));
				} else { $pyjs['__active_exception_stack__'] = $pyjs['__last_exception_stack__']; $pyjs['__last_exception_stack__'] = null; throw $pyjs_try_err; }
			}
			return null;
		}
	...
	, 1, [null,null,['self'],['eventCode'],['details']]);
		...
		$method = $pyjs__bind_method2('onRemoteResponse', function(response, request_info) {
			...
			if ($p['bool']($p['op_eq']($p['getattr'](request_info, 'method'), 'getOccupancies'))) {
				...
				$m['ErrorMessage']($p['__op_add']($add23='Received an unexpected response for method ',$add24=$p['getattr'](request_info, 'method')));
			}
			return null;
		}
	, 1, [null,null,['self'],['response'],['request_info']]);
		...
		$method = $pyjs__bind_method2('onRemoteError', function(code, message, request_info) {
			...
			var $add25,$add26;
			$m['ErrorMessage']($p['__op_add']($add25='Unable to contact server: ',$add26=$p['str'](message)));
			return null;
		}
